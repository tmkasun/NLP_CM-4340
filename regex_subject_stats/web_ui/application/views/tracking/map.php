...
<head>
    ...
    <script>
        ...
        function add_markers(student){

            var student_marker = L.marker(student.location.location.coordinates.reverse(), {icon: normalIcon}).addTo(map);
            student_marker.reg_number = student.reg_number;
            student_marker.on('click', function (marker) {
                ...
                $.get('http://bigdata.knnect.local/index.php/Vehicle_tracking/get_student/'+reg_number, function (data) {
                    ...
                    popupTemplate.find('#department_code').html(data.department_code);
                    popupTemplate.find('#student_name').html(data.name.with_initials);
                    popupTemplate.find('#home_town').html(data.location.asciiname);
                    marker.target.bindPopup(popupTemplate.html()).openPopup();
                });
            });
        }
...
<div style="display: none">
    <div id="markerPopup" class="popover top">
        ...
        <div class="popover-content">
            ...
            <p id="student_name" class="bg-primary" style="margin: 0px;padding: 0px;"></p>
            <h6>Department<span class="label label-primary pull-right"><span id="department_code"></span> km/h</span>
            </h6>
            <h6>Home Town<span id="home_town" class="label label-primary pull-right"></span></h6>
